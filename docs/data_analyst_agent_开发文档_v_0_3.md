...
11. 后端 pytest 使用隔离 DuckDB 测试库，当前测试结果为 `149 passed`。
...
7. PR/Push CI 自动运行后端测试、前端构建和 Secret Scan。
8. 手动真实 Qwen 评测能输出质量门禁摘要与可下载报告。
...
3. 多轮分析能力。
4. 安全审计报告。
5. GitHub Actions CI 与真实 Qwen 质量门禁。

真实 Qwen API 端到端演示、调用成本基线、CI 接入和质量门禁均已完成。下一步优先进行性能检查、版本发布，以及把 LLM 指标接入持久化监控。
