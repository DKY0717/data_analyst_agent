...
- 核心路径回归：`pytest backend/tests/test_core_path_cases.py -q`，确认 `backend/evaluation/cases/core_path_cases.yaml` 中的核心路径黄金问题仍然对齐前端推荐问题、面试演示顺序和已有评测 case。
...
- 远端证据：手动真实 Qwen workflow 会在 quality gate 后生成 `security-audit-*.md/json`，和真实评测报告一起作为 artifact 上传。
