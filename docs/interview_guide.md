...
> 构建自然语言驱动的数据分析 Agent，基于 FastAPI + LangGraph + DuckDB + OpenAI 兼容 LLM API 实现 NL2SQL、多轮追问、分层意图解析、Schema Grounding、主动澄清、SQL 安全校验、YAML 数据权限策略、行级 SQL 过滤、执行失败自动修复、SQL 优化建议、LLM 调用可观测性和结构化审计报告；用 65 条结构化评测、5 条权限回归评测、581 个后端测试、54 个前端单测、17 个 E2E 和 GitHub Actions 质量门禁证明核心链路稳定。
...
当前项目有 581 个后端测试、54 个前端单测、17 个 E2E、65 条结构化评测用例和 5 条数据权限回归评测。基础 CI 不依赖真实 LLM secret，会运行后端测试、PostgreSQL 测试、前端单元测试、前端生产构建、Playwright 前端 E2E、Docker Compose 配置校验、Docker 镜像构建、后端容器 readiness smoke test、tracked-file secret scan、Intent/Grounding/Permission 确定性评测和 quality gate。真实模型评测保留在手动 workflow，避免外部 API 波动拖垮普通 push/PR。
...
先用 analyst 角色查询销售额，展示行级过滤和 SQL 改写；再查询客户姓名，展示 `block_unauthorized_column` 和 `customers.customer_name`；最后切 admin 证明同一问题可以被授权执行。右侧 AuditPanel 和 `python -m evaluation.security_audit_exporter --write-report` 分别展示在线审计和离线报告。离线报告会在“输入完整性”里明确写出是否未提供真实评测输入；版本交付或面试前复核时可以加 `--fail-on-missing-real-reports` 做严格检查，避免把缺失真实评测报告误当成已通过证据。真实 Qwen workflow 也会在 quality gate 后生成 `security-audit-*.md/json`，远端 artifact 可以直接作为真实模型评测审计包展示。
...
4. 继续提问 `按地区拆一下`，展示多轮追问会复用上一轮上下文。
5. 输入模糊问题如 `帮我分析一下`，展示主动澄清候选，再点击候选恢复执行。
6. 运行 `cd backend && python -m evaluation.security_audit_exporter --write-report`，展示可归档的安全审计 JSON/Markdown；如果要做严格交付检查，补齐真实评测报告后加 `--fail-on-missing-real-reports`。
7. 运行 `python scripts/interview_evidence.py --run-id <github_run_id>`，生成本地验证、远端真实 Qwen artifact 下载和 `security-audit-*.md/json` 检查清单。
