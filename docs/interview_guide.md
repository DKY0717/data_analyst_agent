...
> 构建自然语言驱动的数据分析 Agent，基于 FastAPI + LangGraph + DuckDB + OpenAI 兼容 LLM API 实现 NL2SQL、多轮追问、分层意图解析、Schema Grounding、主动澄清、SQL 安全校验、YAML 数据权限策略、行级 SQL 过滤、执行失败自动修复、SQL 优化建议、LLM 调用可观测性和结构化审计报告；用 65 条结构化评测、5 条权限回归评测、14 条核心路径黄金问题、596 个后端测试、51 个前端单测、17 个 E2E 和 GitHub Actions 质量门禁证明核心链路稳定。
...
2. 运行 `pytest backend/tests/test_core_path_cases.py -q`，确认 `backend/evaluation/cases/core_path_cases.yaml` 中的核心路径黄金问题仍然关联已有 NL2SQL、结果正确性和权限评测 case，且前端推荐问题和演示脚本没有漂移。
...
11. 运行 `python scripts/interview_evidence.py --run-id <github_run_id>`，生成本地验证、远端真实 Qwen artifact 下载和 `security-audit-*.md/json` 检查清单。
