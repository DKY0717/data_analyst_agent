...
> 构建自然语言驱动的数据分析 Agent，基于 FastAPI + LangGraph + DuckDB + Qwen API 实现 NL2SQL、多轮追问、SQL 安全校验、执行失败自动修复、结果解释、SQL 优化建议、离线评测报告和安全审计报告；使用 SQLGlot AST 拦截危险 SQL，基于 ContextVar 隔离并汇总模型调用 Token、耗时与可选成本，后端 149 个 pytest 用例与 GitHub Actions 质量门禁覆盖关键路径。
...
后端有 pytest 自动化测试，目前 149 个用例通过。测试覆盖 SQL Guard、安全边界、AgentGraph、LLM usage 解析、并发调用轨迹隔离、执行失败修复、Schema Loader、语义层、两套 Evaluation Runner、ReportWriter、多轮上下文、AuditReport、Query API、QueryRunner、SQL Optimizer 和 CI 门禁。PR 会自动运行后端测试、前端构建与 Secret Scan；真实 Qwen Plus 评测使用独立手动 workflow，避免不稳定外部调用阻塞普通 PR。

质量门禁不会只看“工作流绿不绿”，而是读取真实评测报告，检查正常分析执行率、危险请求阻断率、安全预期命中率和 Repair 端到端成功率。默认模式只告警，手动开启 enforce 后指标下降会直接失败，并始终保留报告 artifact 供复盘。
...
我在 Qwen API 边界读取 DashScope 返回的真实 usage，并记录每次逻辑调用的节点、模型、Token、耗时和 API 尝试次数。因为 Qwen 客户端是全局单例，我使用 `ContextVar` 隔离并发异步请求，再通过 LangGraph state 跨节点汇总到 `audit_report`。模型单价通过环境变量配置，未配置时成本为 `null`，避免硬编码过期价格。本次 32 条 Qwen Plus 基线平均每题调用 `1.78` 次、消耗 `1889` Token、LLM 耗时 `9.30 秒`；一次 Repair 平均增加约 `876 Token` 和 `3.86 秒`。
