...
> 构建自然语言驱动的数据分析 Agent，基于 FastAPI + LangGraph + DuckDB + Qwen API 实现 NL2SQL、多轮追问、三层安全治理、执行失败自动修复、结果解释、SQL 优化建议和审计报告；使用 10 条人工审核黄金 SQL 验证结果正确性，将真实 Qwen Plus 正确率从 5/10 提升到 10/10，并以 300+ pytest 用例和 GitHub Actions 覆盖关键路径。
...
后端有 300+ pytest 自动化测试。PR 会自动运行后端测试、前端构建、Secret Scan 和不依赖外部服务的 Intent Evaluation；真实 Qwen Plus 评测使用独立手动 workflow，避免不稳定外部调用阻塞普通 PR。
