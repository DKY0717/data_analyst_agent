...
> 构建自然语言驱动的数据分析 Agent，基于 FastAPI + LangGraph + DuckDB + Qwen API 实现 NL2SQL、多轮追问、SQL 安全校验、执行失败自动修复、结果解释、SQL 优化建议、离线评测报告和安全审计报告；使用 SQLGlot AST 拦截危险 SQL，基于 ContextVar 隔离并汇总模型调用 Token、耗时与可选成本，后端 124 个 pytest 用例覆盖关键路径。
...
后端有 pytest 自动化测试，目前 124 个用例通过。测试覆盖 SQL Guard、安全边界、AgentGraph、LLM usage 解析、并发调用轨迹隔离、执行失败修复、Schema Loader、语义层、两套 Evaluation Runner、ReportWriter、多轮上下文、AuditReport、Query API、QueryRunner 和 SQL Optimizer。为了避免依赖本机真实数据库，pytest 使用隔离 DuckDB 测试库。
