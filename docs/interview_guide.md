...
> 构建自然语言驱动的数据分析 Agent，基于 FastAPI + LangGraph + DuckDB + Qwen API 实现 NL2SQL、多轮追问、三层安全治理、执行失败自动修复、结果解释、SQL 优化建议和审计报告；使用确定性 Intent Guard 在模型调用前阻断危险意图，使用 SQLGlot AST 拦截危险 SQL，后端 216 个 pytest 用例与 GitHub Actions 100% 安全质量门禁覆盖关键路径。
...
后端有 pytest 自动化测试，目前 216 个用例通过。PR 会自动运行后端测试、前端构建、Secret Scan 和不依赖外部服务的 Intent Evaluation；真实 Qwen Plus 评测使用独立手动 workflow，避免不稳定外部调用阻塞普通 PR。
...
- **可评测**：固定 NL2SQL case 与确定性 Repair 故障集能分别量化生成、安全和修复表现。
