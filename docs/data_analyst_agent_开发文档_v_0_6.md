...
- v0.6 消融实验：`full` 分层预期满足率为 `100%`；`without_clarification` 为 `85.7%`，主动澄清带来 `14.3` 个百分点提升。
...
3. **从能跑到可证明**：用 459 个测试、固定评测集、消融实验、黄金正确性基准和 CI 门禁证明安全、修复、业务口径与 Grounding 质量。

## 后续方向

- 扩充分层意图/Grounding 专用评测集，加入更多口语化、多意图和冲突候选 case，让消融实验覆盖更强的歧义样本。
