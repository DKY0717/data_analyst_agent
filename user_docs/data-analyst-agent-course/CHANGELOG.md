...
> 项目随后补充了可执行核心路径 Runner、PostgreSQL Alembic 往返迁移、demo/secure Compose 边界、Ruff/ESLint/覆盖率与依赖审计门禁，以及安全的 SpreadsheetML 导出。课程首页、代码映射和第 4、15～19 章已同步这些当前实现；章节中的历史基线说明改为以 `v1.7` 开发说明、源码和测试为事实来源。

| 变化 | 对应章节 |
|---|---|
| DuckDB 固定重建，PostgreSQL 才使用 Alembic | 第3、4、18、19章 |
| Fake LLM + 真实 Agent/Guard/权限/隔离数据库核心路径 | 第16、17、19章 |
| secure Compose 缺失强配置时 fail-closed | 第13、18、19章 |
| 前端 SpreadsheetML 文本导出，避免公式注入 | 第15、16章 |
| CI 增加覆盖率、Lint、迁移和依赖审计 | 第16、18章 |
