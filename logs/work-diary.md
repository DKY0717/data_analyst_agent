...
- 还未进入实现阶段；需要先基于设计写实施计划。
- 实施时需注意复用已有 `ecommerce_nl2sql_cases.yaml` 和 `golden_result_cases.yaml`，不要重复维护业务口径。
...
- ✅ v1.6 设计规格已创建
- ⏳ 待写实施计划
- ⏸️ 待实现核心路径配置、加载器、测试、前端推荐问题和演示预检更新

### 下一步

- 基于 v1.6 设计规格创建实施计划。
- 按计划以 TDD 方式实现 `core_path_cases.yaml`、加载器和一致性测试。
- 实现后运行 focused 测试、相关前端测试和 `git diff --check`。
