...
## 2026-06-29 — v0.9 权限演示 E2E 闭环设计

### 完成的工作

- 用户确认下一版先做 v0.9 E2E 权限演示闭环。
- 明确 E2E 使用 Mock 后端响应，不依赖真实 Qwen/MiMo API。
- 抽查现有 Playwright 配置和基础 E2E：当前只覆盖页面结构、基础查询状态、主题切换和响应式布局。
- 新增设计规格：`docs/superpowers/specs/2026-06-29-v0.9-permission-demo-e2e-design.md`。
...
## 2026-07-10 — 核心路径打磨规划会话

### 完成的工作

**v1.6 核心路径打磨设计** ✅
- 用户确认按“黄金问题集、回归测试、失败分类、可信度展示、演示体验”的路线继续推进。
- 梳理现有资产：32 条 NL2SQL 评测、黄金结果基准、权限评测、前端推荐问题、面试证据脚本和演示预检脚本。
- 新增设计规格：`docs/superpowers/specs/2026-07-10-v1.6-core-path-polish-design.md`。
- 明确 v1.6 聚焦统一核心路径黄金问题包、轻量回归校验、前端推荐问题对齐和演示预检顺序，不重写 LangGraph、不新增真实 LLM 普通 CI 调用。
...
- 文档同步和最终验证仍在进行中。
- 实施时已复用已有 `ecommerce_nl2sql_cases.yaml` 和 `golden_result_cases.yaml`，没有重复维护业务口径。
...
- ⏳ 阶段三 SQL 安全、权限与超时硬化进行中

### 下一步

- 钳制用户/模型已有 LIMIT，并补强权限行过滤的别名与歧义处理。
- 为直接 QueryRunner 调用与容器部署补齐可验证的超时边界。
...
- 正式编写课程正文前，需要用户审阅并确认书面设计。
- 工作树出现与本次教程设计无关的前端测试改动，未修改、未暂存、未提交。
...
- 用户确认设计后，先创建课程首页、侧边栏、Docsify 阅读入口、更新日志和代码映射表。
- 按章节批次编写、验证、复核并提交十九章正文。
